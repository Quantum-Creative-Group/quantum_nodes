...
class RunQGAN(bpy.types.Node, AnimationNode):
    ...
    def execute(self): # , trainingObject
        ...
        mu = 1
        sigma = 1
        real_data = np.random.lognormal(mean=mu, sigma=sigma, size=N) # the mean and standard deviation are not the values for the distribution itself, but of the underlying normal distribution it is derived from
