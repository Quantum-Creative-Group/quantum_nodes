...
class SchrodingerEquationSimulationNode(Node, AnimationNode):
    ...
    def execute(self, frame_rate, duration, delta_t, dimension, size,
                center, number_of_waves, sprawl, potential, obstacles):
        try:
            self.sse.updateSimulation(dimension, size, center, number_of_waves, sprawl,
                                      potential, obstacles, frame_rate, duration, delta_t)
